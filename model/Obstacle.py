	...
	def __init__(self, world, pos=None):
		...
		if pos is None:
			self.pos = [world.getHubs()[0].getPosition()[0] + (np.random.randint(-MAX_SEARCH_DIST, MAX_SEARCH_DIST)),
						world.getHubs()[0].getPosition()[1] + (np.random.randint(-MAX_SEARCH_DIST, MAX_SEARCH_DIST))]  # Where the obstacle will be placed
		else:
			self.pos = pos
	...
	def obstruct(self):
		...
		for agent in self.agentNeighbors:
			if agent.getRect().colliderect(self.obstacleRect):
				if not agent.getPrevRect().colliderect(self.obstacleRect):  # If the agent is colliding with obstacle for first time
